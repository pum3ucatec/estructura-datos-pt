...
Este proyecto implementa una **lista simplemente enlazada** en C# utilizando la convención **PascalCase**.  
Incluye las operaciones básicas y un **menú interactivo** en consola para que el usuario pueda interactuar con la lista.

---

## Operaciones implementadas
- `InsertarInicio(int Valor)` → Inserta un nodo al inicio de la lista.  
- `InsertarFinal(int Valor)` → Inserta un nodo al final de la lista.  
- `Eliminar(int Valor)` → Elimina un nodo por su valor.  
- `Buscar(int Valor)` → Retorna `true` si encuentra el valor, `false` en caso contrario.  
- `Mostrar()` → Muestra los elementos de la lista en consola.  
...
Ingrese el valor a insertar al inicio: 10
